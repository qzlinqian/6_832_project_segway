...
def program_for_forward(prog, state, torque, seg_world, time_interval, time_steps):
...
def program_for_circle(prog, state, torque, seg_world, time_interval, time_steps):
    # prog = MathematicalProgram()

    # initial state constraints
    prog.AddConstraint(state[0][0] == 1)
    prog.AddConstraint(state[0][1] == 4)
    prog.AddConstraint(state[0][2] == np.pi)
    prog.AddConstraint(state[0][3] == 0)
    prog.AddConstraint(state[0][4] == 0)
    prog.AddConstraint(state[0][5] == 0)
    prog.AddConstraint(state[0][6] == 0)
    ...
    prog.AddConstraint(state[-1][1] == 3)
    ...
    # distance to reference trajectory
    for t in range(time_steps):
        prog.AddCost(seg_world.deviation_with_ref(state[t]) * 10)
...
if __name__ == '__main__':
    ...
    # seg_world = world.ForwardWorld(5)
    seg_world = world.CircleWorld()
    fig, ax = plt.subplots()
    ...
    # program_for_forward(prog, state, torque, seg_world, time_interval, time_steps)
    program_for_circle(prog, state, torque, seg_world, time_interval, time_steps)
    ...
    np.save('../model/data/torque0.npy', torque_opt)
